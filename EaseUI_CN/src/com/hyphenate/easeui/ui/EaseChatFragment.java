 ...
 * 
 */
public class EaseChatFragment extends EaseBaseFragment implements
		EMMessageListener {
	...
	protected void setUpView() {
		...
		if (chatType == EaseConstant.CHATTYPE_SINGLE) {
			...
			if (EaseUserUtils.getUserInfo(toChatUsername) != null) {
				titleBar.setTitle(EaseUserUtils.getUserInfo(toChatUsername)
						.getNick());
			}
	...
	protected void onConversationInit() {
		...
		// the number of messages loaded into conversation is
		// getChatOptions().getNumberOfMessagesLoaded
	...
	protected void onMessageListInit() {
		messageList.init(
				toChatUsername,
				chatType,
				chatFragmentHelper != null ? chatFragmentHelper
		...
		messageList
				.setItemClickListener(new EaseChatMessageList.MessageListItemClickListener() {
					...
					public void onResendClick(final EMMessage message) {
						new EaseAlertDialog(getActivity(), R.string.resend,
								R.string.confirm_resend, null,
								new AlertDialogUser() {
									@Override
			...
			if (requestCode == REQUEST_CODE_CAMERA) { // capture new image
				...
				if (locationAddress != null && !locationAddress.equals("")) {
					...
					Toast.makeText(getActivity(),
							R.string.unable_to_get_loaction, Toast.LENGTH_SHORT)
							.show();
				}
	...
	protected void addChatRoomChangeListenr() {
		...
		EMClient.getInstance().chatroomManager()
				.addChatRoomChangeListener(chatRoomChangeListener);
	}
		...
		if (EMClient.getInstance().getCurrentUser().equals(group.getOwner())
			...
			message.setAttribute(
					...
					EaseAtMessageHelper.get().atListToJsonArray(
							EaseAtMessageHelper.get().getAtMessageUsernames(
									content)));
		}
		sendMessage(message);
	...
	protected void selectPicFromCamera() {
		...
		cameraFile = new File(PathUtil.getInstance().getImagePath(), EMClient
					...
					public void onResult(boolean confirmed, Bundle bundle) {
						if (confirmed) {
							EMClient.getInstance().chatManager()
									.deleteConversation(toChatUsername, true);
							messageList.refresh();
						}
					}
				}, true).show();
		...
		case TXT:
			if (forward_msg.getBooleanAttribute(
				...
				sendBigExpressionMessage(
						((EMTextMessageBody) forward_msg.getBody())
								.getMessage(),
		...
		case IMAGE:
			...
			if (filePath != null) {
				...
				if (!file.exists()) {
					...
					filePath = ((EMImageMessageBody) forward_msg.getBody())
							.thumbnailLocalPath();
				}
