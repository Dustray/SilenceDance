...
public class KeepBodyActivity extends Activity {
	...
	Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			NewAdapter adapter = new NewAdapter(context, R.layout.new_list_item,
					newLists); // 关联数据和子布局
			lvKeepBodyNew.setAdapter(adapter);
		};
	};
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_keep_body);
		...
		//列表每一项点击事件
		lvKeepBodyNew.setOnItemClickListener(new OnItemClickListener() {
			@Override
			public void onItemClick(AdapterView<?> arg0, View arg1, int position,
					long id) {
				NewEntity one=newList.get(position);
				Intent intent=new Intent(KeepBodyActivity.this,NewInfoActivity.class);
	...
	// 初始化控件
	private void init() {
		lvKeepBodyNew = (ListView) findViewById(R.id.lvKeepBodyNew);
	}
