...
public class MedicalCareActivity extends Activity {
	...
	Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			NewAdapter adapter = new NewAdapter(context, R.layout.new_list_item,
					newLists); // 关联数据和子布局
			lvMedicalCareNew.setAdapter(adapter);
		};
	};
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//列表每一项点击事件
		lvMedicalCareNew.setOnItemClickListener(new OnItemClickListener() {
			@Override
			public void onItemClick(AdapterView<?> arg0, View arg1, int position,
					long id) {
				NewEntity one=newList.get(position);
				Intent intent=new Intent(MedicalCareActivity.this,NewInfoActivity.class);
	...
	// 初始化控件
	private void init() {
		lvMedicalCareNew = (ListView) findViewById(R.id.lvMedicalCareNew);
	}
