...
import com.example.alert.TimeSetDialog;
...
public class ClockEditActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		timeSetButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				timeSetDialog = new TimeSetDialog(ClockEditActivity.this);
				...
				timeSetDialog
						.setOnCancelListener(new DialogInterface.OnCancelListener() {
							...
							public void onCancel(DialogInterface dialog) {
								...
								if (alerttime != null)
									alertTextView.setText(Utils
											.timeTransfer(alerttime));
								else
		...
		// String minute =
		// time.minute<10?"0"+time.minute:String.valueOf(time.minute);
	...
	public void onBackPressed() {
		...
		if ((!content.isEmpty() && !tempContent.equals(content))
				|| !alerttime.equals("") && !alerttime.equals(tempAlerttime)) { // 如果内容非空且已经被更改，则更新显示和数据库
			ArrayList<HashMap<String, String>> list = Utils.getList();
			SqliteUtils sqlite = new SqliteUtils();
			DatabaseHelper dbHelper = sqlite
					.createDBHelper(ClockEditActivity.this);
	...
	private void alertSet() {
		...
		PendingIntent pendingIntent = PendingIntent.getBroadcast(
				ClockEditActivity.this, 0, intent, 0);
